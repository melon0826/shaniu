...
```js
const schema = shaniuCreateSchema.object({
  host: shaniuCreateSchema.string()
    .setTitle("服务地址")
    .setDefault("http://127.0.0.1:9090"),
  enabled: shaniuCreateSchema.boolean()
    .setTitle("启用")
    .setDefault(false),
});

const ConfigDB = new ShaniuPluginConfig(schema);
ConfigDB.get();
...
先在 Admin 面板「yyb-go」中添加名称和地址。脚本里按页面编号创建实例：

```js
const sc = new YybGo({ id: 1 });
```
...
| `userList()` | 获取用户列表 |
| `getCode(options)` | 获取小程序 code，参数 `{ openid, appid }`，返回 yybgo API 原始 JSON |
| `getUserInfo(options)` | 获取小程序用户信息，参数 `{ openid, appid }`，返回 yybgo API 原始 JSON |
| `getPhoneNumber(options)` | 获取手机号 code，调用 `POST /wx/getphonenumber`，返回 yybgo API 原始 JSON |
| `qrCodeAuth(options)` | 二维码 OAuth 授权，调用 `POST /wx/qrcodeauth`，返回 yybgo API 原始 JSON |
| `oAuth(options)` | OAuth 授权，调用 `POST /wx/oauth`，返回 yybgo API 原始 JSON |
| `request(method, path, body, query)` | 调用其他 yybgo API |
...
| 插件配置 | 支持 `shaniuCreateSchema` / `new ShaniuPluginConfig(schema)` / `form(schema)` 声明式配置表单 |
...
| yyb-go | 可添加多个 yyb-go 面板，并在脚本中通过 `new YybGo({ id })` 调用 |
...
- 未设置 `shaniu.password` 时，首次打开后台会强制创建管理员账号和密码。
...
```text
adapters/pagermaid/shaniuplus.py
```

接入步骤：

1. 将 [shaniuplus.py](adapters/pagermaid/shaniuplus.py) 放到 Pagermaid 的插件目录。
2. 把文件里的 `uri = "${rws()}"` 改成 Shaniu 提供的 WebSocket 地址。
3. 重启 Pagermaid，或在 Pagermaid 中重新加载插件。
4. 在 Telegram 里发送 Pagermaid 命令 `shaniu`，返回 `傻+ 已连接` 表示桥接在线。

WebSocket 地址格式：

```text
ws://<Shaniu地址>:8080/<你的WebSocket路径>
```
...
当前仓库提供的是 Pagermaid 端桥接脚本；Shaniu 侧需要有对应的 WebSocket 插件或接口来处理这个路径。没有配置服务端 WebSocket 路由时，Pagermaid 端会一直离线或重连。

更多细节见 `docs/` 目录。

## 致谢

本项目基于并延续了前作者 cdle 的 Shaniu 项目思想与历史代码积累，感谢原项目作者及社区贡献者的长期工作。

- 原项目：https://github.com/cdle/shaniu
