...
本文档深入解析 Shaniu 的内部架构、核心模块设计与数据流。
...
Bucket 是 Shaniu 的统一存储抽象，设计灵感来自 AWS S3 的 Bucket 概念。
...
gRPC 服务使 Shaniu 可以被其他语言编写的客户端调用。

**服务列表**：

| 服务 | 文件 | 说明 |
|------|------|------|
| `ShaniuService` | `grpc_sender.go` | 主服务，包含所有 RPC |
...
Message Received
  → HandleMessage()
    ...
    → for _, function := range Functions
      ...
      → if matched:
        → exec node --require shaniu-runtime-preload.js <plugin.js>
