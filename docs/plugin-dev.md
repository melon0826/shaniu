...
Shaniu 的插件系统基于外部 NodeJS 运行时。插件以 `.js` 文件形式放在 `/data/plugins`，通过顶部注释声明元数据，由框架自动加载、匹配，并通过 gRPC 与 Go 主程序通信。
...
`@web` 只支持 `true` 或 `false`。写 `@web true` 后，Shaniu 会在启动或脚本重载时把该脚本作为常驻 NodeJS 进程运行；HTTP 端口、路由前缀和监听逻辑全部由脚本内 Express 自己决定。
...
### shaniuCreateSchema / ShaniuPluginConfig

Shaniu 支持声明式插件配置。插件可以用 `shaniuCreateSchema` 构造 JSON Schema，再用
`new ShaniuPluginConfig(schema)` 或 `form(schema)` 绑定当前插件配置。后台会在「插件配置」页面展示已注册的配置表单。
...
const schema = shaniuCreateSchema.object({
  ...
  open: shaniuCreateSchema.boolean()
    .setTitle("启用开关")
    .setDefault(false),
  delTime: shaniuCreateSchema.number()
    .setTitle("撤回时间")
    .setDescription("0 表示不撤回")
    .setDefault(0),
  mode: shaniuCreateSchema.string()
    .setTitle("模式")
    .setEnum(["normal", "fast"])
    .setEnumNames(["普通", "快速"]),
});
...
注意：配置 schema 会在插件执行到 `new ShaniuPluginConfig(schema)` 或 `form(schema)` 时注册。新插件首次安装后，
...
`YybGo` 是 yyb-go 面板的脚本内联客户端。先在 Admin 面板左侧「yyb-go」中添加名称和地址，再在脚本里按页面表格编号创建实例。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | number/string | 是 | yybgo 页面中的顺序编号，从 `1` 开始 |
...
| `request(method, path, body, query)` | 任意 yybgo API | 自定义方法、路径、请求体、查询参数 | 原始 API 响应 |

yybgo 运行时不会改写 API 返回。脚本收到的就是 yybgo 原始 JSON，一般结构为：
...
- `new YybGo({ id: 1 })` 只接受对象参数，不支持 `new YybGo(1)`。
- `addUser` 只接受对象参数，推荐写 `sc.addUser({ code: "xxxxx", type: 1, displayName: "备注" })`。
- 只有网络失败、请求体编码失败、JSON 解析失败这类没有 yybgo 原始响应的情况，运行时才会返回 `{ status: false, message: "..." }`。
...
在 Admin 面板将 `shaniu.debug` 设为 `true`，或在插件中：
