...
本文档提供 Shaniu 暴露的所有编程接口的详细参考，包括 REST API、gRPC API 和 JavaScript 插件 API。
...
Token 获取方式：
- 首次访问 Admin 面板时自动生成
- 存储在 Bucket `shaniu` 的 `api_key` 字段中
...
这是 Shaniu 的 WebSocket 替代实现，基于 HTTP 长轮询：
...
**Go 包**：`github.com/melon0826/shaniu/proto3/srpc`
...
stub = srpc_pb2_grpc.ShaniuServiceStub(channel)

# 获取 Bucket 值
req = srpc_pb2.BucketKeyRequest(name="shaniu", key="port")
