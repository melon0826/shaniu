...
type ContainerKind = 'qinglong' | 'daidai' | 'yyb-go';
...
const legacyContainerPages: ContainerKind[] = ['qinglong', 'daidai', 'yyb-go'];
...
onMounted(() => {
  ...
  window.addEventListener('shaniu:admin-auth-expired', handleAdminAuthExpired);
});

onBeforeUnmount(() => {
  window.removeEventListener('shaniu:admin-auth-expired', handleAdminAuthExpired);
});
...
const protectedStorageBuckets = new Set(['plugins', 'shaniu', 'auths']);
...
const yybgo = reactive({
  ...
  total: 0,
  loading: false,
  editing: null as YybGoPanel | null,
  form: {} as YybGoPanel,
  testing: false,
  saving: false,
});
...
async function loadSettings() {
  ...
  settings.form = {
    ...
    password: '',
    port: Number(data['shaniu.port'] || 8080),
    api_key: data['shaniu.api_key'],
    debug: data['shaniu.debug'] === true || data['shaniu.debug'] === 'true',
    listen_admin: data['shaniu.listen_admin'] !== false && data['shaniu.listen_admin'] !== 'false',
    recall: data['shaniu.recall'],
...
async function saveSettings() {
  ...
  const updates: Record<string, unknown> = {
    'shaniu.name': v.name || '',
    'shaniu.port': v.port || 8080,
    'shaniu.api_key': v.api_key || '',
    'shaniu.debug': !!v.debug,
    'shaniu.listen_admin': !!v.listen_admin,
    'shaniu.recall': v.recall || '',
    ...
    <AntApp>
      <div v-if="!booting && !user" class="login-page">
        <div class="login-card">
          ...
          <template v-else>
            <Typography.Title :level="3" style="margin-top: 0">Shaniu Admin</Typography.Title>
          ...
          <main class="content">
            <section v-if="page === 'welcome'" class="panel">
              ...
              <Row :gutter="[12, 12]">
                ...
                <Col :xs="24" :sm="12" :md="8"><Card><Statistic title="yyb-go" :value="overviewIntegrations.find((item) => item.key === 'yyb-go')?.count || 0" /></Card></Col>
              ...
              <Spin :spinning="pluginConfigs.loading">
                <div v-if="pluginConfigs.selected" class="config-form">
                  ...
                  <Alert
                    ...
                    message="该插件检测到配置代码，但安装时没有成功导出配置表单。请确认 new ShaniuPluginConfig(schema) 或 form(schema) 在脚本顶层执行，且脚本初始化没有报错。"
                  />
