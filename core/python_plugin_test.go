...
func TestCreatePythonPluginWritesFlatScriptAndRuntime(t *testing.T) {
	t.Setenv("SHANIU_DATA_PATH", t.TempDir())
	...
	text := string(content)
	for _, want := range []string{"@title PythonSmoke", "from shaniu import sender as s", "asyncio.run(main())"} {
	...
	runtimeDir, err := ensurePythonShaniuModule()
	if err != nil {
		t.Fatal(err)
	}
