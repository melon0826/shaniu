	...
	f.Handle = func(s common.Sender) interface{} {
		...
		case NODE:
			...
			if err := ensureNodeRuntimeDependencies(workDir); err != nil {
				console.Error("NodeJS shaniu 运行时依赖安装失败：%v", err)
				return nil
			}
			var err error
...
interface ShaniuSchemaNode {
	schema: Record<string, any>;
	setTitle(value: string): ShaniuSchemaNode;
	setDescription(value: string): ShaniuSchemaNode;
	setDefault(value: any): ShaniuSchemaNode;
	setEnum(value: any[]): ShaniuSchemaNode;
	setEnumNames(value: string[]): ShaniuSchemaNode;
	setRequired(value: string[] | boolean): ShaniuSchemaNode;
	setFormat(value: string): ShaniuSchemaNode;
	setMin(value: number): ShaniuSchemaNode;
	setMax(value: number): ShaniuSchemaNode;
	setMinLength(value: number): ShaniuSchemaNode;
	setMaxLength(value: number): ShaniuSchemaNode;
	setPattern(value: string): ShaniuSchemaNode;
	setWidget(value: string): ShaniuSchemaNode;
	toJSON(): Record<string, any>;
}
declare const shaniuCreateSchema: {
	string(): ShaniuSchemaNode;
	number(): ShaniuSchemaNode;
	integer(): ShaniuSchemaNode;
	boolean(): ShaniuSchemaNode;
	array(item?: any): ShaniuSchemaNode;
	object(props?: Record<string, any>): ShaniuSchemaNode;
};
declare class ShaniuPluginConfig {
	uuid: string;
...
declare function form(schema: any): ShaniuPluginConfig;
...
func shaniuRuntimeEnv() []string {
	appDir := detectShaniuAppDir()
	latest, source := latestAppVersion()
	values := []string{
		"SHANIU_VERSION=" + currentAppVersion(),
		"SHANIU_REMOTE_VERSION=" + latest,
		"SHANIU_VERSION_SOURCE=" + source,
		"SHANIU_REPOSITORY=" + appRepository,
		"SHANIU_EXEC_PATH=" + currentExecutablePath(),
	}
	if proxy := strings.TrimSpace(githubAcceleratorPrefix()); proxy != "" {
		values = append(values, "SHANIU_GITHUB_PROXY="+proxy)
	}
	if appDir != "" {
		values = append(values, "SHANIU_APP_DIR="+appDir)
	}
	return values
}
...
func detectShaniuAppDir() string {
	wd, _ := os.Getwd()
	candidates := dedupeCleanPaths([]string{
		os.Getenv("SHANIU_APP_DIR"),
		wd,
		utils.ExecPath,
	...
	for _, candidate := range candidates {
		if repo := gitTopLevel(candidate); repo != "" && looksLikeShaniuRepo(repo) {
			return repo
		}
		if looksLikeShaniuRepo(candidate) {
			return candidate
		}
	}
	if wd != "" {
...
func looksLikeShaniuRepo(dir string) bool {
	if strings.TrimSpace(dir) == "" {
		return false
	}
	required := []string{
		...
		filepath.Join(dir, "proto3", "shaniu.js"),
	}
