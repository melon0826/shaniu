...
const defaultPluginSourceRepo = "https://github.com/melon0826/shaniu_Plugins"
...
func initWebPluginList() {
	...
	GinApi(PUT, "/api/admin/plugins/github-proxy", RequireAuth, func(ctx *gin.Context) {
		...
		shaniu.Set(pluginSourceGithubProxyKey, proxy)
...
func pluginSourceAddresses() []string {
	raw := strings.TrimSpace(shaniu.GetString(pluginSourceReposKey))
	if raw == "" {
...
func savePluginSourceAddresses(sources []string) {
	shaniu.Set(pluginSourceReposKey, string(utils.JsonMarshal(sources)))
}
...
func linkPluginSourceItems(address string) ([]*common.Function, error) {
	...
	req.Header.Set("User-Agent", "shaniu")
...
func installGithubNodePlugin(address string) error {
	...
	if class == NODE {
		if err := ensureNodeShaniuModule(target); err != nil {
			return err
		}
		if err := ensureNodePackageJSON(target, "shaniu-plugins"); err != nil {
			return err
		}
	} else if _, err := ensurePythonShaniuModule(); err != nil {
		return err
	}
...
func githubAcceleratorPrefix() string {
	return strings.TrimSpace(shaniu.GetString(pluginSourceGithubProxyKey))
}
