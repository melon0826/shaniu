...
func init() {
	storage.Watch(shaniu, "name", func(old, new, key string) *storage.Final {
		if old == new {
	...
	password = shaniu.GetString("password")
	var name = shaniu.GetString("name", "傻妞")
	// if password == "" {
	// password = utils.GenUUID()
	// console.Info("可视化面板临时账号密码：%s %s", name, password)
	// }
	storage.Watch(shaniu, "password", func(old, new, key string) *storage.Final {
	...
	GinApi(POST, "/api/admin/register", func(ctx *gin.Context) {
		...
		shaniu.Set("name", payload.Username)
		shaniu.Set("password", payload.Password)
	...
	pluginNextUuid := shaniu.GetString("pluginNextUuid")
	if pluginNextUuid == "" {
...
func countOnlineYybGoPanels(panels []YybGoPanel) int {
	count := 0
	for _, panel := range panels {
		if panel.Status == "online" {
			count++
...
func parseAdminJWT(token string) (*adminJWTClaims, error) {
	...
	if currentName := strings.TrimSpace(shaniu.GetString("name")); currentName != "" && claims.Sub != currentName {
...
func adminJWTSecret() []byte {
	sum := sha256.Sum256([]byte(GetMachineID() + "|" + password + "|shaniu-admin-jwt"))
	return sum[:]
}
...
func adminCookieSecure(ctx *gin.Context) bool {
	if strings.EqualFold(strings.TrimSpace(shaniu.GetString("secure_cookie")), "true") ||
		strings.EqualFold(strings.TrimSpace(os.Getenv("SHANIU_SECURE_COOKIE")), "true") {
		return true
	}
