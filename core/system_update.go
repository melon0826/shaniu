...
func init() {
	GinApi(GET, "/api/health", func(ctx *gin.Context) {
		ApiOK(ctx, map[string]interface{}{
			...
			"started_at": shaniu.GetString("started_at"),
		})
	})
...
func updateFromRelease(progress func(int, string)) (*systemUpdateResult, error) {
	...
	tmpDir, err := os.MkdirTemp("", "shaniu-update-*")
	if err != nil {
		return nil, err
	}
...
func fetchLatestRelease() (*releasePayload, error) {
	address := "https://api.github.com/repos/melon0826/shaniu/releases/latest"
	var lastErr error
...
func releaseExecutablePath() (string, error) {
	if configured := strings.TrimSpace(os.Getenv("SHANIU_EXEC_PATH")); configured != "" {
...
func findReleaseBinary(root string) (string, error) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, err error) error {
		...
		name := entry.Name()
		if !strings.HasPrefix(name, "shaniu_") {
			return nil
		}
	...
	if found == "" {
		return "", fmt.Errorf("Release 包中没有找到 shaniu 可执行文件")
	}
	return found, nil
}
...
func findReleaseProto3(root string) (string, error) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, err error) error {
		...
		if entry.Name() == "proto3" {
			if _, statErr := os.Stat(filepath.Join(path, "shaniu.js")); statErr == nil {
				found = path
			}
		}
		return nil
