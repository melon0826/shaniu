...
func TestBackendVersionStorageKeysAreReserved(t *testing.T) {
	for _, key := range []string{"compiled_at", "latest_version", "remote_version", "version"} {
		if !isBackendVersionStorageKey("shaniu", key) {
			t.Fatalf("expected shaniu.%s to be reserved", key)
		}
	...
	keys := filterBackendVersionStorageKeys("shaniu", []string{"name", "version", "remote_version", "port"})
