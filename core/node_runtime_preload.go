...
(function () {
  if (process.env.SHANIU_CONFIG_REGISTER_ONLY === "true") {
    ...
    class ShaniuPluginConfig {
      constructor(schema) {
        ...
        try {
          const fs = require("fs");
          const target = process.env.SHANIU_CONFIG_SCHEMA_FILE || "";
          if (target) fs.writeFileSync(target, JSON.stringify(this.jsonSchema));
          else console.log("__SHANIU_CONFIG_SCHEMA__" + JSON.stringify(this.jsonSchema));
          process.exit(0);
        } catch (err) {
          console.error("ShaniuPluginConfig schema export failed:", err && err.message ? err.message : err);
          process.exit(1);
        }
      }
    }
    function form(schema) { return new ShaniuPluginConfig(schema); }
  ...
  let sg;
  try {
    sg = require(require("path").join(process.cwd(), "node_modules", "shaniu"));
  } catch (error) {
    try {
      sg = require("shaniu");
    } catch (_) {
  ...
  class YybGo {
    ...
    async init(options) {
      const panels = await readPanels("yybgo_panels");
      const index = panelIndex(options);
      if (index < 1 || index > panels.length) throw new Error("yyb-go 编号 " + (index || "") + " 不存在");
      this.panel = panels[index - 1];
      this.id = index;
      this.uuid = this.panel.id || "";
      this.name = this.panel.name || "";
    ...
    async request(method, path, body, query) {
      ...
      try { return JSON.parse(text); } catch (err) {
        ...
        throw new Error("yyb-go 接口返回非 JSON：" + String(text || "").slice(0, 120));
      }
    }
  ...
  sg.YybGo = sg.YybGo || YybGo;
  sg.DaiDai = sg.DaiDai || DaiDai;
  sg.shaniuCreateSchema = sg.shaniuCreateSchema || shaniuCreateSchema;
  ...
  globalThis.YybGo = sg.YybGo;
  globalThis.DaiDai = sg.DaiDai;
  globalThis.shaniuCreateSchema = sg.shaniuCreateSchema;
  globalThis.ShaniuPluginConfig = sg.ShaniuPluginConfig;
