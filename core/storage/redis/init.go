...
var toMaster = func(bucket, key, value string) (string, bool, error) {
	...
	web_token := shaniu.GetString("web_token")
	if web_token == "" {
...
func Try(RedisAddr, RedisPassword string) error {
	...
	err := db.HSet(context.Background(), "shaniu", "storage", "redis").Err()
	if err == nil {
		db.Close()
	}
	return err
...
func Initshaniu(RedisAddr, RedisPassword string) storage.Bucket {
	db = redis.NewClient(&redis.Options{
		Addr:        RedisAddr,
		Password:    RedisPassword, // no password set
		DB:          0,             // use default DB
	...
	port := shaniu.GetString("port", "8080")
	if utils.SlaveMode {
