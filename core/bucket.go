...
import (
	...
	"github.com/melon0826/shaniu/core/storage/boltdb"
	"github.com/melon0826/shaniu/core/storage/redis"
...
func MakeBucket(name string) storage.Bucket {
	...
	if bkt == nil {
		...
		bkt = boltdb.Initshaniu()
		// Get = boltdb.Get
		// Set = boltdb.Set
		// logs.Info("默认使用boltdb进行数据存储")
		// } else {
		// 	bkt = redis.Initshaniu(Config.RedisAddr, Config.RedisPassword)
		// 	Get = redis.Get
		// 	Set = redis.Set
		...
		if def := bkt.GetString("storage"); def == "redis" {
			func() {
				...
				bkt = redis.Initshaniu(app.GetString("redis_addr"), app.GetString("redis_password"))
			}()
		} else {
			if def != "boltdb" {
