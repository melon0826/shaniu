...
func TestRegisterNodePluginConfigSchema(t *testing.T) {
	...
	script := `
const { shaniuCreateSchema, ShaniuPluginConfig } = require("shaniu");
new ShaniuPluginConfig(shaniuCreateSchema.object({
  token: shaniuCreateSchema.string().setTitle("Token").setDefault("abc")
}));
`
...
func TestRegisterPythonPluginConfigSchema(t *testing.T) {
	...
	script := `
from shaniu import shaniuCreateSchema, ShaniuPluginConfig

ShaniuPluginConfig(shaniuCreateSchema.object({
    "token": shaniuCreateSchema.string().setTitle("Token").setDefault("abc")
}))
`
	if err := os.WriteFile(scriptPath, []byte(script), 0644); err != nil {
