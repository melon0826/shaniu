...
type normalUserBindings struct {
	...
	YybGoOpenID    string   `json:"yybgo_openid"`
	YybGoOpenIDs   []string `json:"yybgo_openids"`
	UpdatedAt      int64    `json:"updated_at"`
}
...
func init() {
	...
	GinApi(GET, "/api/user/profile", RequireUserAuth, func(ctx *gin.Context) {
		...
		ApiOK(ctx, gin.H{
			...
			"yybgo_panels":  publicYybGoPanels(),
		})
	})
	...
	GinApi(PUT, "/api/user/bind", RequireUserAuth, func(ctx *gin.Context) {
		...
		if !isPublicUserBindingPlatform(payload.Platform) {
			ApiFail(ctx, "普通用户只能绑定 QQ 或 Telegram；yyb-go 请通过扫码登录")
			return
		}
	...
	GinApi(GET, "/api/user/yybgo/panels", RequireUserAuth, func(ctx *gin.Context) {
		ApiOK(ctx, publicYybGoPanels())
	})

	GinApi(POST, "/api/user/yybgo/qr/start", RequireUserAuth, func(ctx *gin.Context) {
		payload := struct {
		...
		panel, err := yybGoPanelByIndex(payload.Panel)
		if err != nil {
			ApiFail(ctx, err.Error())
			return
		}
		body := map[string]interface{}{"type": payload.Type}
		raw, err := requestYybGoJSON(panel, http.MethodPost, "/api/qr/start", body, nil)
		if err != nil {
			ApiFail(ctx, err.Error())
			return
		}
	...
	GinApi(GET, "/api/user/yybgo/qr/status", RequireUserAuth, func(ctx *gin.Context) {
		...
		panel, err := yybGoPanelByIndex(panelIndex)
		if err != nil {
			ApiFail(ctx, err.Error())
			return
		}
		raw, err := requestYybGoJSON(panel, http.MethodGet, "/api/qr/status", nil, map[string]string{"uuid": uuid})
		if err != nil {
			ApiFail(ctx, err.Error())
			return
		}
	...
	GinApi(POST, "/api/user/yybgo/login/confirm", RequireUserAuth, func(ctx *gin.Context) {
		user := currentNormalUser(ctx)
		if user == nil {
			ApiError(ctx, http.StatusUnauthorized, "请先登录")
			return
		...
		raw, err := requestYybGoJSON(panel, http.MethodGet, "/api/qr/status", nil, map[string]string{"uuid": payload.UUID})
		if err != nil {
			ApiFail(ctx, err.Error())
			return
		}
		...
		addRaw, err := requestYybGoJSON(panel, http.MethodPost, "/api/accounts/add", gin.H{
			"code":        wxCode,
			"uuid":        payload.UUID,
		...
		if openid == "" {
			ApiFail(ctx, "yyb-go 已确认扫码，但添加账号接口未返回 openid")
			return
		}
		bindings, err := updateNormalUserBinding(user.Username, "yybgo", openid)
		if err != nil {
			ApiFail(ctx, err.Error())
			return
		}
	...
	GinApi(POST, "/api/user/yybgo/account/add", RequireUserAuth, func(ctx *gin.Context) {
		user := currentNormalUser(ctx)
		if user == nil {
			ApiError(ctx, http.StatusUnauthorized, "请先登录")
			return
		...
		raw, err := requestYybGoJSON(panel, http.MethodPost, "/api/accounts/add", gin.H{
			"code":        payload.Code,
			"type":        payload.Type,
		...
		if openid == "" {
			ApiFail(ctx, "yyb-go 添加账号成功，但接口未返回 openid")
			return
		}
		bindings, err := updateNormalUserBinding(user.Username, "yybgo", openid)
		if err != nil {
	...
	GinApi(POST, "/api/user/yybgo/code", RequireUserAuth, func(ctx *gin.Context) {
		user := currentNormalUser(ctx)
		if user == nil {
			ApiError(ctx, http.StatusUnauthorized, "请先登录")
			return
		...
		if !normalUserHasYybGoOpenID(user.Username, payload.OpenID) {
			ApiFail(ctx, "只能为当前用户已绑定的 yyb-go openid 生成 code")
			return
		}
		panel, err := yybGoPanelByIndex(payload.Panel)
		if err != nil {
		...
		raw, err := requestYybGoJSON(panel, http.MethodPost, "/wx/code", gin.H{
...
func userJWTSecret() []byte {
	sum := sha256.Sum256([]byte(GetMachineID() + "|shaniu-normal-user-jwt"))
	return sum[:]
}
...
func updateNormalUserBinding(username string, platform string, value string) (normalUserBindings, error) {
	...
	case "yybgo", "yybgo_openid":
		if value == "" {
			bindings.YybGoOpenID = ""
			bindings.YybGoOpenIDs = nil
			break
		}
		bindings.YybGoOpenIDs = appendUniqueOpenID(bindings.YybGoOpenIDs, value)
		if bindings.YybGoOpenID == "" {
			bindings.YybGoOpenID = bindings.YybGoOpenIDs[0]
		}
	default:
...
func normalizeNormalUserBindings(bindings normalUserBindings) normalUserBindings {
	...
	openids := []string{}
	if text := strings.TrimSpace(bindings.YybGoOpenID); text != "" {
		openids = appendUniqueOpenID(openids, text)
	}
	for _, openid := range bindings.YybGoOpenIDs {
		openids = appendUniqueOpenID(openids, openid)
	}
	bindings.YybGoOpenIDs = openids
	if len(openids) > 0 {
		bindings.YybGoOpenID = openids[0]
	} else {
		bindings.YybGoOpenID = ""
	}
	return bindings
...
func normalUserHasYybGoOpenID(username string, openid string) bool {
	openid = strings.TrimSpace(openid)
	if openid == "" {
		return false
	}
	bindings := loadNormalUserBindings(username)
	for _, item := range bindings.YybGoOpenIDs {
		if item == openid {
			return true
		}
	}
	return bindings.YybGoOpenID == openid
}
