...
const pythonPipxRuntimePackage = "shaniu-python-runtime"
const pythonGrpcRuntimeDependency = "grpcio==1.83.0"
const pythonProtobufRuntimeDependency = "protobuf==7.35.1"

var nodeShaniuRuntimeDependencies = map[string]string{
	"@grpc/grpc-js":   "^1.8.18",
...
var nodeShaniuModuleCache sync.Map
...
func init() {
	...
	GinApi(PUT, "/api/admin/node/dependency/registry", RequireAuth, func(ctx *gin.Context) {
		...
		shaniu.Set("pnpm_registry", registry)
...
func handleSetPluginDependencyRegistry(ctx *gin.Context) {
	...
	if runtime == PYTHON {
		...
		shaniu.Set("pipx_registry", registry)
...
func createNodePlugin(pluginName, title, fileName string, class string) (string, string, error) {
	...
	switch class {
	case NODE:
		if err := ensureNodeShaniuModule(root); err != nil {
			return "", "", err
		}
		if err := ensureNodePackageJSON(root, "shaniu-plugins"); err != nil {
			return "", "", err
		}
...
`
	case PYTHON:
		if _, err := ensurePythonShaniuModule(); err != nil {
			return "", "", err
		}
...
func readNodeDependencies(plugin nodeDependencyPlugin) ([]nodeDependencyRow, error) {
	...
	if err := ensureNodePackageJSON(dir, "shaniu-plugins"); err != nil {
		return nil, err
	}
...
func normalizeDependencyName(value string) string {
	...
	if value == "shaniu" || nodeBuiltinModules[value] {
		return ""
	}
...
var pythonIgnoredModules = map[string]bool{
	...
	"shaniu": true, "srpc-pb2": true, "srpc-pb2-grpc": true,
}
...
func ensureNodePackageJSON(dir, pluginName string) error {
	...
	manifest := nodeDependencyManifest{
		...
		Version:      "1.0.0",
		Private:      true,
		Dependencies: nodeShaniuRuntimeDependencyCopy(),
	}
...
func ensureNodeShaniuModule(dir string) error {
	dir = filepath.Clean(dir)
	if _, ok := nodeShaniuModuleCache.Load(dir); ok {
		return nil
	}
	...
	moduleDir := filepath.Join(nodeModules, "shaniu")
	if err := os.MkdirAll(moduleDir, 0755); err != nil {
		return err
	}
	if err := copyNodeRuntimeFile("shaniu.js", filepath.Join(moduleDir, "index.js")); err != nil {
		return err
	}
	if err := copyNodeRuntimeFile("srpc.js", filepath.Join(moduleDir, "srpc.js")); err != nil {
		return err
	}
	if err := writeFileIfChanged(filepath.Join(moduleDir, "shaniu.d.ts"), []byte(typeat), 0644); err != nil {
		return err
	}
	if err := writeFileIfChanged(filepath.Join(nodeModules, "shaniu.d.ts"), []byte(typeat), 0644); err != nil {
		return err
	}
	packageJSON := []byte(`{"name":"shaniu","main":"index.js","types":"shaniu.d.ts","private":true}
`)
	if err := writeFileIfChanged(filepath.Join(moduleDir, "package.json"), packageJSON, 0644); err != nil {
		return err
	}
	nodeShaniuModuleCache.Store(dir, true)
	return nil
}
...
func copyNodeRuntimeFile(name, target string) error {
	...
	return fmt.Errorf("缺少 NodeJS shaniu 运行时文件：%s", name)
}
...
func nodeShaniuRuntimeDependencyCopy() map[string]string {
	deps := map[string]string{}
	for name, version := range nodeShaniuRuntimeDependencies {
		deps[name] = version
	}
	return deps
}
...
func ensurePipxRuntimePackage() (string, error) {
	...
	pkg := filepath.Join(root, "shaniu_python_runtime")
	if err := os.MkdirAll(pkg, 0755); err != nil {
		return "", err
	}
	pyproject := `[project]
name = "shaniu-python-runtime"
version = "1.0.0"
requires-python = ">=3.12"

[project.scripts]
shaniu-python-runtime = "shaniu_python_runtime:main"
`
	initPy := `def main():
    return 0
`
...
func resolvePipxCommand() (pipxCommand, error) {
	...
	pipxCommandCache.Unlock()
	if env := strings.TrimSpace(os.Getenv("SHANIU_PIPX")); env != "" {
		bin, args := splitCommand(env)
		if bin == "" {
			return pipxCommand{}, errors.New("SHANIU_PIPX 为空")
		}
		if !commandWorks(bin, args, "--version") {
			return pipxCommand{}, errors.New("SHANIU_PIPX 不能执行 pipx")
		}
...
func nodeRuntimeNodePath() string {
	key := os.Getenv("SHANIU_NODE_PATH") + "\x00" + os.Getenv("NODE_PATH")
...
func nodeRuntimeModulePaths() []string {
	candidates := []string{}
	for _, env := range []string{os.Getenv("SHANIU_NODE_PATH"), os.Getenv("NODE_PATH")} {
...
func resolvePnpmCommand() (pnpmCommand, error) {
	...
	pnpmCommandCache.Unlock()
	if env := strings.TrimSpace(os.Getenv("SHANIU_PNPM")); env != "" {
...
func pnpmRegistry() string {
	registry := strings.TrimSpace(shaniu.GetString("pnpm_registry"))
	if registry == "" {
...
func pipxRegistry() string {
	registry := strings.TrimSpace(shaniu.GetString("pipx_registry"))
	if registry == "" {
...
func resolveNodeCommand() (string, error) {
	...
	nodeCommandCache.Unlock()
	if env := strings.TrimSpace(os.Getenv("SHANIU_NODE")); env != "" {
