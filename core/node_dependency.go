...
func ensureNodeSillygirlModule(dir string) error {
	...
	if err := os.WriteFile(filepath.Join(moduleDir, "shaniu.d.ts"), []byte(typeat), 0644); err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(nodeModules, "shaniu.d.ts"), []byte(typeat), 0644); err != nil {
		return err
	}
