...
var pythonShaniuModuleCache sync.Map
...
func ensurePythonShaniuModule() (string, error) {
	if configured := strings.TrimSpace(os.Getenv(		"SHANIU_PYTHON_PATH")); configured != "" {
		...
		pythonShaniuModuleCache.Store(configured, true)
		return configured, nil
	}
	dir := filepath.Join(utils.ExecPath, "language", "python")
	if _, ok := pythonShaniuModuleCache.Load(dir); ok {
		return dir, nil
	}
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
	for _, name := range []string{"shaniu.py", "srpc_pb2.py", "srpc_pb2_grpc.py"} {
		if err := copyPythonRuntimeFile(name, filepath.Join(dir, name)); err != nil {
			return "", err
		}
	}
	pythonShaniuModuleCache.Store(dir, true)
	return dir, nil
}
...
func validatePythonRuntimePath(dir string) error {
	for _, name := range []string{"shaniu.py", "srpc_pb2.py", "srpc_pb2_grpc.py"} {
		...
		if err != nil || info.IsDir() {
			return fmt.Errorf("Python shaniu 运行时文件不存在：%s", path)
		}
	}
	return nil
}
...
func resolvePythonCommand() (string, []string, error) {
	...
	if configured := strings.TrimSpace(os.Getenv("SHANIU_PYTHON_BIN")); configured != "" {
		bin, args := splitCommand(configured)
		if bin == "" {
			return "", nil, errors.New("SHANIU_PYTHON_BIN 为空")
		}
		if pythonCommandVersion(bin, args) == pythonRequiredVersion {
			cachePythonCommand(bin, args)
			return bin, args, nil
		}
		return "", nil, fmt.Errorf("SHANIU_PYTHON_BIN 必须指向 Python %s", pythonRequiredVersion)
	}
...
func registerPythonPluginConfigSchema(path, uuid string) error {
	...
	temp, err := os.CreateTemp("", "shaniu-python-plugin-schema-*.json")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
	...
	cmd.Env = append(os.Environ(),
		pythonRuntimeEnvVars(pythonPath)...,
	)
