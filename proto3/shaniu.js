...
async function updateGit(options, timeout) {
    const repo = await resolveShaniuRepo(options.appDir, timeout);
...
async function updateDocker(options, timeout, gitError) {
    ...
    const watchtowerImage = String(options.dockerWatchtowerImage || process.env?.SHANIU_WATCHTOWER_IMAGE || "containrrr/watchtower:latest").trim();
    await pullDockerImage(socket, watchtowerImage, timeout);
    const helperName = `shaniu-watchtower-${Date.now()}`;
    const create = await dockerJSON(socket, "POST", `/containers/create?name=${encodeURIComponent(helperName)}`, {
        ...
        Labels: {
            "shaniu.update": "watchtower",
            "shaniu.target": containerName,
        },
        HostConfig: {
            AutoRemove: true,
...
async function resolveShaniuRepo(configured, timeout) {
    const candidates = [];
    addRepoCandidate(candidates, configured);
    addRepoCandidate(candidates, process.env?.SHANIU_APP_DIR);
    addRepoCandidate(candidates, process.env?.SHANIU_HOME);
    ...
    addRepoCandidate(candidates, "/data/shaniu");
    for (const candidate of candidates) {
        try {
            const result = await git(candidate, ["rev-parse", "--show-toplevel"], timeout);
            const repo = result.stdout.trim();
            if (repo && await isShaniuRepo(repo, timeout))
                return repo;
        }
        catch (_) { }
    }
    throw new Error("未找到可更新的 Shaniu Git 仓库；Docker/Release 部署请使用镜像或 Release 包更新");
}
async function isShaniuRepo(repo, timeout) {
    try {
        const result = await git(repo, ["config", "--get", "remote.origin.url"], timeout);
        const remote = result.stdout.trim().toLowerCase();
        return remote.includes("shaniu") && !remote.includes("shaniu_plugins");
    }
    catch (_) {
        return false;
    }
