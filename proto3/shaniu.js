...
let client = new srpc_1.srpc.ShaniuServiceClient(process.env?.SHANIU_GRPC_ADDR || "127.0.0.1:50051", grpc_1.credentials.createInsecure());
let senders = [];
...
exports.shaniuCreateSchema = shaniuCreateSchema;
class ShaniuPluginConfig {
    uuid = plugin_id;
    jsonSchema;
    userConfig = {};
    ready;
...
exports.ShaniuPluginConfig = ShaniuPluginConfig;
function form(schema) {
    return new ShaniuPluginConfig(schema);
}
async function readRuntimePanels(key) {
    const raw = await new Bucket("shaniu").get(key, []);
...
class YybGo {
    ...
    async init(options) {
        const panels = await readRuntimePanels("yybgo_panels");
        const index = runtimePanelIndex(options);
        if (index < 1 || index > panels.length) {
            throw new Error(`yybgo 编号 ${index || ""} 不存在`);
        }
        this.panel = panels[index - 1];
        this.id = index;
        this.uuid = this.panel.id || "";
    ...
    async request(method, path, body, query) {
        ...
        try {
            ...
            throw new Error("yybgo 接口返回非 JSON：" + String(text || "").slice(0, 120));
        }
    }
    createQr(type) {
...
async function updateRelease(options, timeout) {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "shaniu-update-"));
    const archive = path.join(tmpDir, safeFileName(asset.name || "shaniu-release"));
    try {
...
async function fetchReleaseMetadata(options, timeout) {
    const repo = String(options.releaseRepo || process.env?.SHANIU_RELEASE_REPO || "melon0826/shaniu").trim();
...
function releaseExecutablePath(options) {
    const configured = String(options.executablePath || process.env?.SHANIU_EXEC_PATH || "").trim();
    if (configured)
        return path.resolve(configured);
    if (process.platform === "win32")
        return path.resolve(process.cwd(), "shaniu.exe");
    return "/app/shaniu";
}
...
async function installReleasePayload(tmpDir, executablePath) {
    ...
    if (!binary)
        throw new Error("Release 包中没有找到 shaniu 可执行文件");
...
function findReleaseBinary(root) {
    ...
    return walkFiles(root).find((file) => {
        ...
        return name.startsWith("shaniu_") && (suffix ? name.endsWith(suffix) : !name.endsWith(".zip") && !name.endsWith(".tar.gz"));
    });
}
function findReleaseProto3(root) {
    const dirs = walkDirs(root);
    return dirs.find((dir) => path.basename(dir) === "proto3" && fs.existsSync(path.join(dir, "shaniu.js")));
}
...
async function curlText(urls, timeout, extraArgs = []) {
    ...
    for (const url of urls) {
        try {
            const result = await runCommand("curl", ["-fsSL", "--retry", "2", "--connect-timeout", "8", "--max-time", String(timeout), "-H", "User-Agent: shaniu", ...extraArgs, url], timeout);
...
async function curlDownload(urls, target, timeout) {
    ...
    for (const url of urls) {
        try {
            await runCommand("curl", ["-fL", "--retry", "2", "--connect-timeout", "8", "--max-time", String(timeout), "-H", "User-Agent: shaniu", "-o", target, url], timeout);
            return;
        }
        catch (error) {
            lastErr = error;
...
function releaseGithubProxyPrefixes() {
    const configured = String(process.env?.SHANIU_GITHUB_PROXY || "").trim();
