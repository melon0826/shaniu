...
grpc_addr = os.environ.get("SHANIU_GRPC_ADDR", "127.0.0.1:50051")
grpc_token = os.environ.get("SHANIU_GRPC_TOKEN", "")
metadata = (("runtime_id", runtime_id), ("shaniu-runtime-token", grpc_token))
...
def get_stub():
    ...
    if _stub is None:
        ...
        _stub = srpc_pb2_grpc.ShaniuServiceStub(_channel)
    return _stub
...
def get_async_stub():
    ...
    if _async_stub is None:
        ...
        _async_stub = srpc_pb2_grpc.ShaniuServiceStub(_async_channel)
    return _async_stub
...
class YybGo:
    ...
    async def _ready(self):
        if self.panel is not None:
            return
        panels = await _read_runtime_panels("yybgo_panels")
        if self.id < 1 or self.id > len(panels):
            raise RuntimeError(f"yyb-go 编号 {self.id or ''} 不存在")
        self.panel = panels[self.id - 1]
        self.uuid = self.panel.get("id", "")
        self.name = self.panel.get("name", "")
        self.address = str(self.panel.get("address", "")).rstrip("/")
